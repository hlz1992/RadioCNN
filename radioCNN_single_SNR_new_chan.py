...
class RadioCNN(object):
    ...
    def eval_layer_outputs(self, X, Y):
        conv1_flat_ou, fc1_ou, fc2_ou, cnn_ou = self.sess.run(
            [self.conv1_flat, self.fc1_o, self.fc2_o, self.y_conv],
            feed_dict={self.x: X, self.y_: Y}
        )

        return conv1_flat_ou, fc1_ou, fc2_ou, cnn_ou
